...
HEDE.Game = function (){
    ...
    var game = {
        ...
        //nodeList: document.getElementsByClassName("windowContent"),
        ...
        renderMemoryCard: function (i) {
            ...
            game.gameBoard.appendChild(aTag);
                ...
                if(game.gameCards[i].clickable === true){
                    ...
                    if(game.twoClicks==2){
                        ...
                        if(game.twoClicks==2){
                            ...
                            game.gameBoard.appendChild(pAttempt);
                            
                            if(game.totalAttempts > 1){
                                game.gameBoard.removeChild(oldAttemptText);
                            }
                        ...
                        if(game.totalWins == game.gameCards.length/2){
                            ...
                            game.gameBoard.appendChild(doneMessageP);
                        }
                    }
                }
            };
    ...
    var rows = 4;
    var cols = 4;
    var gameCards = HEDE.RandomGenerator.getPictureArray(rows, cols);
    var count= 0;

    
    game.gameBoard.className = "memoryBoard";

    game.lastNode.appendChild(game.gameBoard);
    
    
    //Loop som huvudsakligen ansvarar för att anropa rendering-funktion till vart och ett av spelkorten.
    for( var i = 0; i < gameCards.length; i++){
        count++; 
        game.gameCards[i] = new HEDE.GameCard(gameCards[i], i);
        game.renderMemoryCard(i);
        
        //Br-tagg läggs till för radbrytning.
        if (count == cols){
            var brTag = document.createElement("br");
            game.gameBoard.appendChild(brTag);
            count=0;
        }
    }
};
